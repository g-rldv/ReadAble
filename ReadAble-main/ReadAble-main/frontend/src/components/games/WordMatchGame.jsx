...
// Mobile: tap a blue word to select it, tap a slot to place it
// Desktop: full HTML5 drag-and-drop + touch drag with ghost
// ============================================================
...
export default function WordMatchGame({ activity, onSubmit, submitting }) {
  ...
  const [selected,  setSelected]  = useState(null);  // tap-to-select on mobile
  ...
  // ── Tap-to-select (mobile primary interaction) ────────────
  const handleTapRight = (value) => {
    if (usedRight.has(value)) { /* already placed — do nothing */ return; }
  ...
  // ── Touch drag (mobile secondary — works alongside tap) ───
    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-2 mb-4">
        ...
        <button onClick={() => speak(content.instruction)}
          className="p-2 rounded-xl text-sky hover:bg-sky/10 transition-colors flex-shrink-0">
          <Volume2 size={16}/>
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        <div className="space-y-3">
          ...
          {content.pairs.map(({ left }) => {
            ...
            const isTarget = !!selected && !matched; // slot available for tap-drop
            return (
              <div key={left}
                data-left-key={left}
...
Drag to match
              ...
              <div key={right}
                ...
                className={`p-3 rounded-2xl border-2 text-sm font-bold text-center
                  ...
                  ${isUsed
                    ...
                    : isSelected
                    ? 'border-sky bg-sky text-white cursor-pointer scale-105 shadow-lg'
                    : 'border-sky/50 bg-sky/10 text-sky cursor-pointer hover:bg-sky/20 hover:border-sky hover:scale-105 active:scale-95'
                  }
      ...
      <div className="mt-4 mb-3">
        <div className="flex justify-between text-xs text-gray-400 mb-1">
          ...
          {allMatched && <span className="text-emerald-500 font-bold flex items-center gap-1"><CheckCircle size={11}/> Ready!</span>}
        </div>
